...
class TareasTab(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        self.add_tarea_btn = ttk.Button(actions_frame, text="Agregar Tarea",
                                        command=self._open_add_tarea_dialog_wrapper, state=tk.NORMAL)
        ...
        self.edit_tarea_btn = ttk.Button(actions_frame, text="Editar Tarea",
                                         command=self._open_edit_tarea_dialog_wrapper, state=tk.NORMAL)
        ...
        self.complete_tarea_btn = ttk.Button(actions_frame, text="Completar",
                                           command=self._mark_selected_tarea_completed_wrapper, state=tk.NORMAL)
        ...
        self.delete_tarea_btn = ttk.Button(actions_frame, text="Eliminar Tarea",
                                           command=self._delete_selected_tarea_wrapper, state=tk.NORMAL)
    ...
    def on_case_changed(self, case_data):
        """Llamado por la ventana contenedora (CaseDetailsWindow) para establecer el caso."""
        self.current_case_data = case_data
        if self.current_case_data:
            case_id = self.current_case_data.get('id')
            self.load_tareas(case_id)
        else:
            self.load_tareas(None) # Limpia la lista si no hay caso
    ...
    def _update_action_buttons_state(self):
        ...
        caso_seleccionado_en_app = self.current_case_data is not None
    ...
    def _open_add_tarea_dialog_wrapper(self):
        if self.current_case_data:  # <-- Usar la variable de la pestaña
            # Llamar a un método en app_controller que abrirá el diálogo
            self.app_controller.open_tarea_dialog(caso_id=self.current_case_data['id'], parent_window=self)
        else:
    ...
    def _open_edit_tarea_dialog_wrapper(self):
        if self.selected_tarea_id:
            self.app_controller.open_tarea_dialog(tarea_id=self.selected_tarea_id, parent_window=self)
        else:
